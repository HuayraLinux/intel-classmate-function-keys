...
static const char *PM_DEV_PATH = "/dev/ipml_pm";

static const char *XRANDR_VGA =
// "xrandr --output LVDS1 --off --output VGA1 --auto";
"/usr/bin/fnkey/cmpcfk-control.sh other";
static const char *XRANDR_LVDS =
"/usr/bin/fnkey/cmpcfk-control.sh lvds";
static const char *XRANDR_DUAL =
"/usr/bin/fnkey/cmpcfk-control.sh dual";
...
Window *get_existing_windows_num(Display *disp,unsigned long *len)
{
...
void pageup(Display *xdpy,KEY_STATUS_MASK mask)
{
	...
	int modstate = 0;
	unsigned long len;
        get_existing_windows_num(xdpy,&len);
        if(2 >= len)
        {
                return;
        }
...
void show_desktop(Display *xdpy,KEY_STATUS_MASK mask)
{
	...
	int rst2 = -1;
	int rst3 = -1;
	rst1 = system("wmctrl -m | grep OFF");
	rst2 = system("wmctrl -m | grep ON");
	rst3 = system("wmctrl -m | grep N/A | grep desktop");
	if (0 == rst1)
		system("wmctrl -k on");
	if (0 == rst2)  
		system("wmctrl -k off");
...
void SetWlanDownPressed(void)
{
	...
	value = system("dbus-send --system --type=method_call --print-reply --dest=org.freedesktop.NetworkManager /org/freedesktop/NetworkManager org.freedesktop.DBus.Properties.Get string:org.freedesktop.NetworkManager string:WirelessEnabled |grep true -q");

	if (!WEXITSTATUS(value))
	{
		// disable wifi
		OnScrDsp::GetIns().ShowWnd(OnScrDsp::WND_WRL, 0);
		ret = system("/usr/sbin/rfkill block wifi");
		set_wireless_data(pm_dev, m_pDriverPath, 0);
	}
	else
	{
		// enable wifi
		OnScrDsp::GetIns().ShowWnd(OnScrDsp::WND_WRL, 1);
		set_wireless_data(pm_dev, m_pDriverPath, 1);
		ret = system("/usr/sbin/rfkill unblock wifi");
...
void SetDisplayDownPressed(void)
{
	...
	ret = system("/usr/bin/fnkey/cmpcfk-control.sh conn");
